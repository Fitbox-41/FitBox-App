...
## ⚠️ FitBox Brand Overrides (LOCKED — these WIN over the generic recommendations below)

The generic engine suggested an orange/green palette and Barlow. FitBox has a real,
locked brand. Use these, not the generic values, whenever they conflict:

- **Brand colour = the FitBox logo red.** `FitBoxColors.logoRed #E31E24`, primary red
  `#D8474D`, dark `#B03A40`. Green (`#19A463`) is used ONLY for wallet credit; debit red
  `#E5484D`. Backgrounds: `bgTopDark #14181C` / `bgBottomDark #0A0C0F` (never pure #000).
- **Typography = Oswald** for display/titles/metrics/labels (both platforms) + platform body
  (SF Pro on iOS, Inter on Android). NOT Barlow. All in `lib/src/core/theme/app_typography.dart`.
- **Everything else from the engine below is adopted:** Modern-Dark "cinema mobile" style,
  glassmorphism, spring + haptic press (scale 0.97→1.0), expo.out `Cubic(0.16,1,0.3,1)`
  easing, staggered reveals (`back.out(1.4)` equivalent → flutter_animate), avoid pure black.
- **The v1.6.1 auth landing is the quality bar for every screen** (see memory `fitbox-ui-standard`).

---

**Project:** FitBox
**Generated:** 2026-07-21 20:07:56
